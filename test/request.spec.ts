...
describe('requests', () => {
  // 测试用例运行前的钩子函数, 执行 jasmine.Ajax.install() 安装 jasmine.Ajax。
  beforeEach(() => {
    jasmine.Ajax.install()
  })

  // 测试用例运行后的钩子函数，执行 jasmine.Ajax.uninstall() 卸载 jasmine.Ajax。
  afterEach(() => {
  ...
  test('should reject when request timeout', done => {
    ...
    getAjaxRequest().then(request => {
       ...
       * 这里用了 request.eventBus.trigger('timeout') 方法触发了 timeout 事件。因为这个方法不在接口定义中，所以需要加 // @ts-ignore。
       */
      // @ts-ignore
